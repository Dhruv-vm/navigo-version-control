      ...
      <div className="grid grid-cols-12 gap-6 mt-6">
        ...
        <div className="col-span-12 lg:col-span-8 space-y-4">
          ...
          {activeLeg && !activeLeg.hasSeatMap ? (
            ...
            <EmptyCabinCard cabinLabel={CABIN_CLASSES.find((c) => c.key === activeCabin)?.label ?? activeCabin} />
          ) : activeLeg ? (
            <SeatMapCard
              leg={activeLeg}
...
// ---------------------------------------------------------------------------
// CabinClassTabs — each class gets its own accent identity (economy=blue,
// premium economy=cyan, business=violet, first=gold) so switching cabins
// feels like switching contexts, not just toggling a filter. Active tab
// gets an animated gradient sweep + glow; price tag pops on selection.
// ---------------------------------------------------------------------------

const CABIN_ACCENTS: Record<CabinKey, { ring: string; text: string; glow: string; sweep: string; dot: string }> = {
  economy: {
    ring: "border-blue-400/70",
    text: "text-blue-300",
    glow: "shadow-[0_0_24px_rgba(96,165,250,0.18)]",
    sweep: "linear-gradient(110deg, transparent, rgba(96,165,250,0.18), transparent)",
    dot: "bg-blue-400",
  },
  premium_economy: {
    ...
    sweep: "linear-gradient(110deg, transparent, rgba(34,211,238,0.18), transparent)",
    dot: "bg-cyan-400",
  },
  business: {
    ...
    sweep: "linear-gradient(110deg, transparent, rgba(167,139,250,0.18), transparent)",
    dot: "bg-violet-400",
  },
  first: {
    ...
    sweep: "linear-gradient(110deg, transparent, rgba(251,191,36,0.25), transparent)",
    dot: "bg-amber-400",
  },
}
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5">
      {cabins.map((c) => {
        ...
        const accent = CABIN_ACCENTS[c.key]
        return (
          <button
            key={c.key}
            onClick={() => onChange(c.key)}
            className={`cabin-tab relative flex flex-col items-start gap-1 px-4 py-3.5 rounded-xl border text-left transition-all duration-200 overflow-hidden ${
              isActive
                ? `${accent.ring} bg-white/[0.04] ${accent.glow}`
                : "border-white/[0.08] bg-white/[0.02] hover:border-white/20 hover:bg-white/[0.05]"
            }`}
          >
            {isActive && (
              <span
                className="cabin-sweep absolute inset-0 pointer-events-none"
                style={{ backgroundImage: accent.sweep }}
              />
            )}

            {isActive && (
              <span className={`absolute top-2.5 right-2.5 w-4 h-4 rounded-full ${accent.dot} flex items-center justify-center seat-pop`}>
                <span className="text-[8px] text-[#060B14] font-bold">✓</span>
              </span>
            )}

            <span className={`relative text-[10px] tracking-widest ${isActive ? accent.text : "text-slate-600"}`}>
              {c.icon}
            </span>
            <span className={`relative text-xs font-semibold ${isActive ? "text-white" : "text-slate-300"}`}>
              {c.label}
            </span>
            <span className={`relative text-[11px] font-medium ${isActive ? accent.text : "text-slate-600"}`}>
              from {c.fromPrice}
            </span>
          </button>
        )
      ...
      {passengers.map((p, i) => {
        ...
        return (
          <button
            key={p.id}
            onClick={() => onSelect(i)}
            className={`pax-tab relative flex items-center gap-2.5 pl-2 pr-3.5 py-2 rounded-xl border text-sm font-medium transition-all duration-200 ${
              isActive
                ? `${color.ring} ${color.bg} ${color.text} ${color.glow}`
                : "border-white/[0.08] text-slate-400 hover:text-slate-200 hover:border-white/20 hover:bg-white/[0.02]"
            }`}
          >
            {/* Avatar with initials + glow ring when active */}
            <span
              className={`relative w-7 h-7 rounded-full flex items-center justify-center text-[9px] font-bold shrink-0 ${
                isActive ? color.dot + " text-white" : "bg-white/10 text-slate-400"
              }`}
            >
              {isActive && <span className={`absolute -inset-1 rounded-full ${color.dot} opacity-30 glow-pulse`} />}
              <span className="relative">{paxInitials(p, i)}</span>
            </span>

            <span>{paxFullName(p, i)}</span>

            {seat ? (
              <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded ${color.badge} ${isActive ? "seat-pop" : ""}`}>
                {seat.seatNumber}
              </span>
            ) : (
...
// Shown when a cabin tab exists (so it's clickable) but generated zero
// seats for this flight instance — e.g. total_seats is 0 or seat_layout
// couldn't be parsed for that class row. Prevents ever silently mixing in
// another cabin's seats under the wrong tab.
function EmptyCabinCard({ cabinLabel }: { cabinLabel: string }) {
  return (
    <div className="bg-[#0D1A2C]/80 border border-white/[0.08] rounded-2xl p-10 text-center">
      ...
      <p className="text-sm text-slate-500 mt-2">Try a different cabin class above.</p>
    </div>
  )
}
...
function SeatMapCard({
  ...
  const cabinAccent = CABIN_ACCENTS[activeCabin]

  return (
    <div
      className="relative bg-gradient-to-br from-[#0D1A2C] via-[#0B1729] to-[#0A1424] border border-white/[0.08] rounded-2xl overflow-hidden shadow-[0_20px_60px_rgba(0,0,0,0.3)]"
    >
      ...
      />
      {/* class-tinted ambient wash */}
      <div className={`pointer-events-none absolute -top-24 left-1/2 -translate-x-1/2 w-[500px] h-[300px] rounded-full blur-[100px] opacity-30`} style={{ background: cabinAccent.dot.includes("amber") ? "rgba(251,191,36,0.4)" : cabinAccent.dot.includes("violet") ? "rgba(167,139,250,0.4)" : cabinAccent.dot.includes("cyan") ? "rgba(34,211,238,0.4)" : "rgba(96,165,250,0.4)" }} />

      {/* Header */}
      <div className="relative flex flex-wrap items-center justify-between px-6 py-4 border-b border-white/[0.06] bg-white/[0.025] gap-3">
        <div className="flex items-center gap-2.5">
          <span className={`w-2.5 h-2.5 rounded-full ${activePaxColor.dot} ${activePaxColor.glow} glow-pulse`} />
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <LegendChip swatch="bg-blue-500/20" label="Available" />
          {visibleSeats.some((s) => s.seatType === "extra_legroom") && (
            <LegendChip swatch="bg-amber-400/20" label="Extra legroom" />
          )}
          <LegendChip swatch="bg-cyan-400" label="Your pick" glow />
          <LegendChip swatch="bg-white/[0.06]" label="Taken" icon="lock" />
        </div>
      </div>

      {/* Aircraft nose indicator */}
      <div className="relative flex justify-center pt-7 pb-3">
        <div className="flex flex-col items-center gap-2.5">
          <div className="relative">
            <span className={`absolute inset-0 rounded-full ${cabinAccent.dot} opacity-40 blur-md nose-beacon`} />
            <svg width="36" height="18" viewBox="0 0 36 18" fill="none" className="relative">
              <path d="M18 0L35 17H1L18 0Z" fill="currentColor" className={`${cabinAccent.text} opacity-25`} />
              <path d="M18 5L27 14H9L18 5Z" fill="currentColor" className={`${cabinAccent.text} opacity-60`} />
            </svg>
          </div>
          <div className="text-slate-500 text-[10px] tracking-[0.22em] uppercase font-medium">Front of aircraft</div>
        </div>
      </div>

      <div className="relative px-6 pb-10 pt-2 overflow-x-auto">
        {isPremiumOrLuxury ? (
          <LuxurySeatGrid
            rows={rows}
                  ...
                  {group.map((seat) => {
                    ...
                    return (
                      <button
                        ...
                        className={`seat-btn relative w-9 h-9 rounded-[9px] flex items-center justify-center transition-all duration-150 group
                          ${isMe
                            ? `${color ? color.bg.replace("/10", "") : "bg-cyan-400"} ${color ? color.glow : "shadow-[0_0_16px_rgba(34,211,238,0.45)]"} scale-110 seat-pop`
                            : isOtherPax && color
...
function LuxurySeatGrid({
  ...
  const sortedCols = [...firstRowSeats].sort((a, b) => a.col.localeCompare(b.col))
  const colGroups = groupByAisle(sortedCols)
  // First class sizes itself via FirstClassPodIcon's own inline dimensions;
  // this is only consumed for business class tiles now.
  const seatSize = "w-[66px] h-[70px]"
        ...
        {colGroups.map((group, gi) => (
          <div key={gi} className="flex items-center gap-3">
            {group.map((s) => (
              <span
                key={s.col}
                className={`inline-flex justify-center text-[11px] font-semibold text-slate-500 ${isFirst ? "w-[104px]" : "w-[66px]"}`}
              >
                {s.col}
              </span>
            ))}
                  ...
                  {group.map((seat, seatIdxInGroup) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={!seat.isAvailable}
                        title={`${seat.seatNumber}${seat.price > 0 ? ` · +₹${seat.price}` : ""}`}
                        className={`seat-btn relative flex flex-col items-center justify-center gap-1.5 transition-all duration-200 group ${isFirst ? "" : seatSize}`}
                      >
                        {isFirst ? (
                          <FirstClassPodIcon
                        ...
                        {seat.isAvailable && !isMe && !isOtherPax && seat.price > 0 && (
                          <span className="absolute top-1 right-1 text-[8px] leading-none px-1 py-0.5 rounded bg-[#060B14]/90 border border-amber-400/30 text-amber-300/80 z-10">
                            +₹{seat.price}
                          </span>
                        )}
                      </button>
...
// ---------------------------------------------------------------------------
// PodIcon — business class seat glyph. Previously built from a separate
// absolutely-positioned "wing" div stacked on the seat body, which floated
// outside the tile and collided with the price badge. Rebuilt as a single
// SVG silhouette (seat back + integrated headrest in one path) so nothing
// pokes outside the tile boundary.
// ---------------------------------------------------------------------------
...
function PodIcon({
  ...
  const tileFillClass = isMe
    ? color ? color.bg.replace("/10", "/20") : "bg-cyan-400/20"
    : isOtherPax && color
    ? color.bg.replace("/10", "/15")
    : !available
    ? "bg-white/[0.025]"
    : "bg-white/[0.045] group-hover:bg-amber-400/[0.08]"

  const tileBorderClass = isMe
    ? color ? color.ring : "border-cyan-400/70"
    : isOtherPax && color
    ? color.ring
    : !available
    ? "border-white/[0.05]"
    : "border-white/[0.1] group-hover:border-amber-400/35"

  const glowClass = isMe ? (color ? color.glow : "shadow-[0_0_18px_rgba(34,211,238,0.35)]") : ""

  const iconColorClass = isMe
    ? color ? color.text : "text-cyan-300"
    : isOtherPax && color
    ? color.text
    : !available
    ? "text-white/15"
    : "text-white/30 group-hover:text-amber-300/60"

  return (
    <div
      className={`relative w-full h-full ${tileFillClass} ${tileBorderClass} ${glowClass} border-2 rounded-2xl flex items-center justify-center transition-all duration-200 group-hover:-translate-y-0.5 overflow-hidden`}
    >
      <svg
        width="60%"
        height="60%"
      ...
      >
        {/* single-piece seat silhouette: back + integrated headrest wing */}
        <path
          d="M6 21V9.5C6 5.9 8.7 3 12.5 3c2.4 0 4.3 1.4 5.2 3.4"
          stroke="currentColor"
          strokeWidth="1.6"
          strokeLinecap="round"
        />
        <path
          d="M6 21h12v-3.2c0-1-.8-1.8-1.8-1.8H7.8c-1 0-1.8.8-1.8 1.8V21z"
          stroke="currentColor"
          strokeWidth="1.6"
          strokeLinejoin="round"
        />
        <path
          d="M17 9.2c1.4 0 2.6 1 2.6 2.5v3.3"
          stroke="currentColor"
          strokeWidth="1.6"
          strokeLinecap="round"
        />
        {available && (
          <path d="M9.5 17.5v2M12 17.5v2M14.5 17.5v2" stroke="currentColor" strokeWidth="1.3" strokeOpacity="0.5" strokeLinecap="round" />
        )}
      </svg>

      {!available && (
        <div className="absolute inset-0 flex items-center justify-center bg-[#070C16]/40">
          <LockIcon size={isFirst ? 20 : 16} className="text-amber-500" filled />
        </div>
      )}
    </div>
  )
...
// ---------------------------------------------------------------------------
// FirstClassPodIcon — purpose-built to match the reference screenshot
// exactly: a gold/bronze metallic outer shell shaped like a rounded shield
// (curved on the aisle side, straighter on the window side), dark purple
// interior, with four distinct interior zones reading top-down:
//   top-left  -> small round screen/mirror
//   top-right -> entertainment display
//   bottom-left -> ottoman/footrest
//   bottom-right -> seat cushion
// and a center-left tray-table square where the orange padlock badge sits
// when the seat is taken — same placement as the reference image.
// ---------------------------------------------------------------------------

function FirstClassPodIcon({
  available,
  isMe,
  isOtherPax,
  color,
  ...
  // Selection state recolors the gold shell + interior tint via Tailwind's
  // `currentColor`, otherwise it stays true to the reference's bronze/purple
  // palette.
  const shellStrokeClass = isMe
    ? color ? color.text : "text-cyan-400"
    : isOtherPax && color
    ? color.text
    : "text-amber-200/70 group-hover:text-amber-200"

  const interiorFill = isMe
    ? color ? color.bg.replace("/10", "/25") : "fill-cyan-400/20"
    : isOtherPax && color
  ...
  return (
    <div
      className={`relative transition-all duration-200 group-hover:-translate-y-1 ${isMe ? "seat-pop" : ""}`}
      style={{
        width: 104,
        height: 96,
        transform: flip ? "scaleX(-1)" : undefined,
        filter: isMe
          ? `drop-shadow(0 0 18px ${color ? "currentColor" : "rgba(34,211,238,0.55)"})`
          : undefined,
      }}
    >
      <svg width="104" height="96" viewBox="0 0 104 96" fill="none" className={shellStrokeClass}>
        {/* outer gold/bronze shield shell */}
        <path
          d="M14 78V40C14 20 28 8 52 8C76 8 90 20 90 40V78C90 84 85 89 79 89H25C19 89 14 84 14 78Z"
          fill={isMe ? "currentColor" : isOtherPax ? "currentColor" : "#1a1430"}
          fillOpacity={isMe || isOtherPax ? 0.12 : 1}
          stroke="currentColor"
          strokeWidth="3"
        />
        {/* inner dark cabin floor */}
        <rect x="22" y="24" width="60" height="56" rx="10" className={interiorFill} />

        {/* top-left round screen/mirror */}
        <circle cx="33" cy="34" r="5" className="fill-white/10" stroke="currentColor" strokeOpacity="0.4" strokeWidth="1" />
        {/* small accent arc near top-center, echoes the reference's thin curve */}
        <path d="M44 30q6 5 12 0" stroke="currentColor" strokeOpacity="0.35" strokeWidth="1.5" strokeLinecap="round" />

        {/* top-right entertainment display, with a soft cyan flicker on hover */}
        <rect x="62" y="26" width="16" height="13" rx="3" className="fill-cyan-400/15 group-hover:fill-cyan-400/30 transition-colors duration-300" stroke="currentColor" strokeOpacity="0.4" strokeWidth="1" />
        <circle cx="70" cy="32.5" r="2.4" className={`fill-cyan-300/60 ${available ? "group-hover:fill-cyan-200" : ""} transition-colors duration-300`} />

        {/* bottom-left ottoman */}
        <rect x="25" y="62" width="16" height="15" rx="4" className="fill-white/[0.06]" stroke="currentColor" strokeOpacity="0.3" strokeWidth="1" />

        {/* bottom-right seat cushion, curved */}
        <path
          d="M52 55c10 0 16 5 16 13v6a4 4 0 01-4 4H56a4 4 0 01-4-4V55Z"
        ...
        {/* center tray-table square, hosting the lock badge or seat letter */}
        <rect
          x="34"
          y="44"
          width="20"
          height="20"
          rx="5"
          className={available ? "fill-white/[0.04]" : "fill-amber-500/10"}
          stroke="currentColor"
          strokeOpacity={available ? 0.3 : 0.6}
          strokeWidth="1.3"
        />
      </svg>
      ...
      {!available && (
        <div
          ...
          style={{ left: 34, top: 44, width: 20, height: 20, transform: flip ? "scaleX(-1)" : undefined }}
        >
          <LockIcon size={16} className="text-amber-500" filled />
        </div>
      )}
      ...
      {isMe && (
        <div
          ...
          style={{ transform: flip ? "scaleX(-1)" : undefined }}
        >
          <span className={`absolute inset-0 rounded-full ${color ? color.dot : "bg-cyan-400"} ${color ? color.glow : "shadow-[0_0_12px_rgba(34,211,238,0.5)]"}`} />
          <span className="relative text-[10px] text-[#060B14] font-bold">✓</span>
        </div>
      )}
    </div>
  )
...
function LegendChip({
  ...
  return (
    <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-white/[0.02]">
      <span className={`relative w-3.5 h-3.5 rounded-[5px] flex items-center justify-center ${swatch} ${glow ? "shadow-[0_0_10px_rgba(34,211,238,0.6)]" : ""}`}>
        {icon === "lock" && <LockIcon size={8} className="text-amber-500/80" />}
      </span>
    ...
    <div className="sticky top-24">
      <div className="relative bg-gradient-to-b from-[#0D1A2C] to-[#0A1424] border border-white/[0.08] rounded-2xl overflow-hidden">
        ...
        <div className="px-6 flex items-end justify-between mb-5">
          ...
          <span className={`text-[28px] leading-none font-semibold tabular-nums text-amber-300 ${totalSeatPrice > 0 ? "seat-pop" : ""}`}>
...
function PageStyles() {
  return (
    <style jsx global>{`
      ...
      @keyframes cabinSweep { from { background-position: -200% 0; } to { background-position: 200% 0; } }
      .cabin-sweep { background-size: 200% 100%; animation: cabinSweep 2.4s linear infinite; }
      ...
      @keyframes noseBeacon { 0%, 100% { opacity: 0.3; transform: scale(1); } 50% { opacity: 1; transform: scale(1.3); } }
      .nose-beacon { animation: noseBeacon 1.6s ease-in-out infinite; }
      ...
      .cabin-tab:hover { transform: translateY(-2px); }
      .pax-tab:hover { transform: translateY(-1px); }
      ...
      @media (prefers-reduced-motion: reduce) {
        .ambient-drift-1, .ambient-drift-2, .ambient-drift-3, .cabin-sweep,
        .glow-pulse, .seat-pop, .nose-beacon, .shimmer-sweep, .rail-glow {
